...
int decode()
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (not correctByte(nbyte))
      {
        printf("incorrect event data or fotter\n");
        return -1;
      }
      if (isFooter())
        break;
      ...
      if ((not getMppcNHitCh(mppc, nMppcHit)) || (not getPmtNHitCh(pmt, nPmtHit)) || (tdcCnt > 2048 && abs(tdcCnt + tdcOffset - preTdcCnt) > pow(2, 16)))
      {
        ...
        int nShift = 3;
        if (__builtin_popcount((int)*tmpBuf[0]) >= 3)
          nShift = 1;
        else if (__builtin_popcount((int)*tmpBuf[1]) >= 3)
          nShift = 2;
        else
          nShift = 3;
      ...
      if ((tdcCnt + tdcOffset) < preTdcCnt && preTdcCnt > (pow(2, 27) - 2048) && tdcCnt < pow(2, 12))
        tdcOffset += pow(2, 27);
      tdcCnt += tdcOffset;
...
int main(int argc, char **argv)
{
  ...
  TString fnameMain = gSystem->BaseName(input);
  fnameMain.ReplaceAll(".dat", "");
  TString dirname = gSystem->DirName(input);
  gSystem->mkdir(dirname + "/root", kTRUE);
  gSystem->mkdir(dirname + "/logs", kTRUE);

  TString output = dirname + "/root/" + fnameMain + ".root";
  TString logfile = dirname + "/logs/" + fnameMain + ".log";
